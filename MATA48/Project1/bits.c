 ...
 * Maicon Douglas Invenção Viana dos Santos
...
int allEvenBits(int x) {
  ...
  IMPAR = (((((IMPAR << 8)+IMPAR)<<8)+IMPAR)<<8)+IMPAR;
...
int logicalShift(int x, int n) {
    ...
    MASK = (((!!n<<31)>>31)&~MASK) | (((!n<<31)>>31)&MASK);
...
int logicalNeg(int x) {
  ...
  return (~((x>>16)|x))&0x00000001;

  //return (((x<<31)>>31)&0) | (((~x<<31)>>31)&1);
...
int twosBits(int x, int n) {
  ...
  //return (~logicalShift(~(x<<(32-n)),32-n)^x)==0;
